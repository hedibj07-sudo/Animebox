...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await fetch(`${JIKAN_BASE}/top/anime?limit=15`);
        const data = await res.json();
        setTopAnime((data.data || []).map(fromJikan));
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`${JIKAN_BASE}/anime?q=${encodeURIComponent(q)}&limit=8&sfw`);
        const data = await res.json();
