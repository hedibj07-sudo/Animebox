...
export default function App() {
    ...
    (async () => {
      ...
      try {
        const [res1, res2] = await Promise.all([
          fetch(`${JIKAN_BASE}/top/anime?limit=25&page=1`),
          fetch(`${JIKAN_BASE}/top/anime?limit=25&page=2`),
        ]);
        const [data1, data2] = await Promise.all([res1.json(), res2.json()]);
        const items = [...(data1.data || []), ...(data2.data || [])].map(fromJikan);
        setTopAnime(items);
      } catch (e) {
        setCatalogError("Impossible de charger le catalogue MyAnimeList pour le moment.");
      }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`${JIKAN_BASE}/anime?q=${encodeURIComponent(q)}&limit=10&sfw=true`);
        if (res.status === 429) {
