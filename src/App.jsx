...
export default function App() {
  ...
  const [showAllAiring, setShowAllAiring] = useState(false);
  ...
  return (
    <div className="st-body" style={{ background: PALETTE.bg, minHeight: "100vh", color: PALETTE.text, paddingBottom: 100 }}>
      ...
      <div style={{ position: "relative", padding: "18px 20px 10px", display: "flex", alignItems: "center", justifyContent: "space-between", overflow: "hidden" }}>
        <div
          aria-hidden="true"
          style={{
            position: "absolute",
            ...
            width: 180,
            height: 180,
            background: "radial-gradient(circle at 35% 35%, rgba(246,197,68,0.4), transparent 60%), radial-gradient(circle at 65% 65%, rgba(88,58,140,0.4), transparent 60%)",
            filter: "blur(28px)",
            pointerEvents: "none",
          }}
        />
        <h1
          ...
          style={{ position: "relative", fontSize: 26, margin: 0, fontWeight: 800, cursor: "pointer" }}
        >
          Sortia
        </h1>
            ...
            {searchQuery.trim().length > 1 ? (
              ...
              <>
                <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 12 }}>
                  ...
                  {topAnime.length > 12 && (
                    <button onClick={() => setShowAllTrending((v) => !v)} className="st-btn" style={{ display: "flex", alignItems: "center", gap: 2, background: "none", border: "none", color: PALETTE.accent, fontSize: 13, fontWeight: 700, cursor: "pointer", padding: 0 }}>
                      {showAllTrending ? "Réduire" : "Tout voir"} <ChevronRight size={14} />
                    </button>
                  )}
                </div>
                ...
                <div className="poster-grid">
                  {(showAllTrending ? topAnime : topAnime.slice(0, 12)).map((anime) => {
                ...
                {(airingNow.length > 0 || airingLoading) && (
                  <>
                    <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", margin: "28px 0 12px" }}>
                      ...
                      {airingNow.length > 12 && (
                        <button onClick={() => setShowAllAiring((v) => !v)} className="st-btn" style={{ display: "flex", alignItems: "center", gap: 2, background: "none", border: "none", color: PALETTE.accent, fontSize: 13, fontWeight: 700, cursor: "pointer", padding: 0 }}>
                          {showAllAiring ? "Réduire" : "Tout voir"} <ChevronRight size={14} />
                        </button>
                      )}
                    </div>
                    {airingLoading && airingNow.length === 0 && <div style={{ fontSize: 13, color: PALETTE.muted, marginBottom: 12 }}>Chargement…</div>}
                    <div className="poster-grid">
                      {(showAllAiring ? airingNow : airingNow.slice(0, 12)).map((anime) => {
                        const entry = entries.find((e) => e.anime_id === anime.id);
                        return (
                          <button key={anime.id} onClick={() => openRating(anime)} className="st-card" style={{ textAlign: "left", cursor: "pointer", border: "none", background: "none", padding: 0, color: PALETTE.text }}>
                            <div style={{ position: "relative", aspectRatio: "2/3", borderRadius: 14, overflow: "hidden", marginBottom: 6 }}>
                  ...
                  <>
                    <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", margin: "28px 0 4px" }}>
                      ...
                      {recommended.length > 12 && (
                        <button onClick={() => setShowAllRecommended((v) => !v)} className="st-btn" style={{ display: "flex", alignItems: "center", gap: 2, background: "none", border: "none", color: PALETTE.accent, fontSize: 13, fontWeight: 700, cursor: "pointer", padding: 0 }}>
                          {showAllRecommended ? "Réduire" : "Tout voir"} <ChevronRight size={14} />
                        </button>
                      )}
                    </div>
                    ...
                    <div className="poster-grid">
                      {(showAllRecommended ? recommended : recommended.slice(0, 12)).map((anime) => (
                        <button key={anime.id} onClick={() => openRating(anime)} className="st-card" style={{ textAlign: "left", cursor: "pointer", border: "none", background: "none", padding: 0, color: PALETTE.text }}>
                          <div style={{ position: "relative", aspectRatio: "2/3", borderRadius: 14, overflow: "hidden", marginBottom: 6 }}>
                            <Poster image={anime.image} genre={anime.genre} />
                          </div>
        ...
        {tab === "profil" && (
          <>
            ...
            <button
              ...
              className="st-btn"
              style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", marginTop: 28, background: PALETTE.surface, border: "none", borderRadius: 14, padding: "14px 16px", cursor: "pointer", textAlign: "left" }}
            >
