...
// Badges débloqués selon les habitudes de visionnage (calculés côté client à partir de `entries`)
const BADGES = [
  { key: "first", label: "Premier pas", desc: "Note ton premier anime", icon: Star, check: (s) => s.total >= 1 },
  { key: "ten", label: "Cinéphile", desc: "10 anime notés", icon: Library, check: (s) => s.total >= 10 },
  { key: "fifty", label: "Marathonien", desc: "50 anime notés", icon: Trophy, check: (s) => s.total >= 50 },
  { key: "critic", label: "Critique", desc: "10 commentaires écrits", icon: MessageCircle, check: (s) => s.comments >= 10 },
  { key: "optimist", label: "Optimiste", desc: "Moyenne des notes ≥ 4", icon: Sparkles, check: (s) => s.avg >= 4 },
  { key: "explorer", label: "Explorateur", desc: "5 genres différents notés", icon: Compass, check: (s) => s.genreCount >= 5 },
  { key: "binger", label: "Binge-watcher", desc: "500 épisodes suivis au total", icon: PlayCircle, check: (s) => s.totalEpisodes >= 500 },
  { key: "social", label: "Sociable", desc: "Au moins 1 ami ajouté", icon: Users, check: (s) => s.friends >= 1 },
];
...
export default function App() {
  ...
  const [discoverPending, setDiscoverPending] = useState(null);
  const [dragX, setDragX] = useState(0);
  const [dragAnim, setDragAnim] = useState(false);
  const dragState = React.useRef({ startX: 0, dragging: false });
  ...
  /* --- avis aléatoires de la communauté (changent à chaque ouverture) --- */
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const userIds = [...new Set(shuffled.map((e) => e.user_id))];
        let profMap = {};
        if (userIds.length > 0) {
          const { data: profs } = await supabase.from("profiles").select("*").in("id", userIds);
          profMap = Object.fromEntries((profs || []).map((p) => [p.id, p]));
        }
        setCommunityComments(shuffled.map((e) => ({ ...e, profile: profMap[e.user_id] || null })));
      } catch (e) {
        // section discrète, pas d'erreur bloquante si ça rate
      }
  ...
  /* --- ouvrir un profil partagé via lien (?profil=pseudo) --- */
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const shared = params.get("profil");
    if (!shared) return;
    window.history.replaceState(null, "", window.location.pathname);
    (async () => {
      const { data } = await supabase.from("profiles").select("*").eq("username", shared).maybeSingle();
      if (data) viewProfile(data);
    })();
  }, []);
  ...
  async function shareProfile() {
    if (!myProfile) return;
    const url = `${window.location.origin}${window.location.pathname}?profil=${encodeURIComponent(myProfile.username)}`;
    if (navigator.share) {
      try {
        await navigator.share({ title: "Sortia", text: `Découvre mon profil Sortia @${myProfile.username}`, url });
      } catch (e) {}
    } else {
      try {
  ...
  /* --- Découverte façon "swipe" --- */
  async function loadDiscoverDeck() {
    setDiscoverLoading(true);
    setDiscoverError("");
    setDiscoverPending(null);
    setDragX(0);
    setDragAnim(false);
    try {
      ...
      if (json.errors) throw new Error(json.errors[0].message);
      const ratedIds = new Set(entries.map((e) => e.anime_id));
      const candidates = groupSeasons((json.data.Page.media || []).map(fromAniList)).filter((a) => !ratedIds.has(a.id));
      const shuffled = [...candidates].sort(() => Math.random() - 0.5).slice(0, 10);
      if (shuffled.length === 0) throw new Error("Aucune proposition trouvée");
      setDiscoverDeck(shuffled);
      setDiscoverIndex(0);
    } catch (e) {
      setDiscoverError("Impossible de charger des propositions, réessaie.");
    }
  ...
  function onCardPointerUp() {
    ...
    const threshold = 90;
    if (dragX > threshold) flySwipe(1);
    else if (dragX < -threshold) flySwipe(-1);
    else {
      setDragAnim(true);
      setDragX(0);
    }
  ...
  function flySwipe(direction) {
    setDragAnim(true);
    setDragX(direction * 600);
    setTimeout(() => {
      const anime = discoverDeck[discoverIndex];
      if (direction > 0 && anime) setDiscoverPending(anime);
  ...
  async function quickAddToWatchlist(anime) {
  ...
  function rateFromDiscover(anime) {
      ...
      <div className="st-body" style={{ background: PALETTE.bg, minHeight: "100vh", color: PALETTE.text, padding: "48px 20px", display: "flex", alignItems: "center" }}>
        ...
        <div style={{ maxWidth: 420, margin: "0 auto", width: "100%" }}>
          ...
          <form onSubmit={saveUsername} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <input type="text" name="username" autoComplete="off" value={usernameDraft} onChange={(e) => setUsernameDraft(e.target.value)} placeholder="ex: yuki_anime" style={{ padding: "13px 16px", borderRadius: 14, border: `1px solid ${PALETTE.line}`, background: PALETTE.surface, color: PALETTE.text, fontSize: 14 }} />
  ...
  const filteredEntries = (() => {
    const base = collectionFilter === "tous" ? entries : entries.filter((e) => e.status === collectionFilter);
    const sorted = [...base];
    if (collectionSort === "alpha") sorted.sort((a, b) => a.anime_title.localeCompare(b.anime_title, "fr"));
    else if (collectionSort === "rating") sorted.sort((a, b) => (Number(b.rating) || 0) - (Number(a.rating) || 0));
    else if (collectionSort === "year") sorted.sort((a, b) => (Number(b.anime_year) || 0) - (Number(a.anime_year) || 0));
    ...
    return (
      <div className="st-body" style={{ background: PALETTE.bg, minHeight: "100vh", color: PALETTE.text, paddingBottom: 100 }}>
        ...
        <div style={{ padding: "16px 20px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {rel.type === "guest" && (
            <button onClick={() => setAuthModalOpen(true)} className="st-btn" style={{ background: PALETTE.accent, border: "none", borderRadius: 999, padding: "9px 14px", color: PALETTE.accentText, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
              Connexion
            </button>
          )}
          {rel.type === "none" && (
    ...
    <div className="st-body" style={{ background: PALETTE.bg, minHeight: "100vh", color: PALETTE.text, paddingBottom: 100 }}>
          ...
          <>
            ...
            {searchQuery.trim().length > 1 ? (
              ...
              <>
                ...
                {(communityComments.length > 0 || communityCommentsLoading) && (
                  <>
                    <h2 className="st-title" style={{ fontSize: 20, margin: "28px 0 12px", fontWeight: 800, display: "flex", alignItems: "center", gap: 8 }}>
                      <Quote size={18} color={PALETTE.muted} /> Ils en parlent
                    </h2>
                    {communityCommentsLoading && communityComments.length === 0 && <div style={{ fontSize: 13, color: PALETTE.muted, marginBottom: 12 }}>Chargement…</div>}
                    <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 6, marginRight: -20, paddingRight: 20 }}>
                      {communityComments.map((c) => (
                        <div key={c.id} style={{ flex: "0 0 240px", background: PALETTE.surface, borderRadius: 16, padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 34, height: 34, borderRadius: 8, overflow: "hidden", flexShrink: 0 }}>
                              <Poster image={c.anime_image} genre={c.anime_genre} />
                            </div>
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontSize: 12, fontWeight: 700, color: PALETTE.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.anime_title}</div>
                              <div style={{ fontSize: 11, color: PALETTE.muted }}>{c.profile ? `@${c.profile.username}` : "Un utilisateur"}</div>
                            </div>
                          </div>
                          {c.rating ? <Stars value={Number(c.rating)} readOnly size={12} /> : null}
                          <p style={{ fontSize: 12, color: PALETTE.muted, margin: 0, lineHeight: 1.4, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{c.comment}</p>
                        </div>
                      ))}
                    </div>
                  </>
          ...
          <>
            ...
            <div style={{ marginBottom: 16 }}>
              <select
                ...
                style={{ padding: "8px 12px", borderRadius: 10, border: `1px solid ${PALETTE.line}`, background: PALETTE.surface, color: PALETTE.text, fontSize: 12, fontWeight: 700 }}
              >
                <option value="recent">Trier : récemment ajouté</option>
                <option value="alpha">Trier : alphabétique (A-Z)</option>
                <option value="rating">Trier : meilleure note</option>
                <option value="year">Trier : année (récent d'abord)</option>
              </select>
            </div>
            {entriesLoading ? (
              <div style={{ textAlign: "center", padding: 60, color: PALETTE.muted }}>Chargement…</div>
        ...
        {tab === "profil" && (
          <>
            ...
            {(() => {
              ...
              const commentsCount = entries.filter((e) => e.comment && e.comment.trim().length > 0).length;
              const totalEpisodes = entries.reduce((s, e) => s + (e.current_episode || 0), 0);
              const badgeStats = {
                total,
                avg: avg === "–" ? 0 : Number(avg),
              ...
              return (
                <>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    <div style={{ fontSize: 12, color: PALETTE.muted, fontWeight: 700, marginBottom: 12 }}>
                      RÉCOMPENSES ({BADGES.filter((b) => b.check(badgeStats)).length}/{BADGES.length})
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
                      {BADGES.map((badge) => {
                        const unlocked = badge.check(badgeStats);
                        const BadgeIcon = badge.icon;
                        return (
                          <div key={badge.key} title={badge.desc} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, opacity: unlocked ? 1 : 0.35 }}>
                            <div style={{ width: 52, height: 52, borderRadius: "50%", background: unlocked ? PALETTE.gold : PALETTE.surface, display: "flex", alignItems: "center", justifyContent: "center" }}>
                              <BadgeIcon size={22} color={unlocked ? "#FFFFFF" : PALETTE.muted} />
                            </div>
                            <div style={{ fontSize: 10, fontWeight: 700, textAlign: "center", lineHeight: 1.2 }}>{badge.label}</div>
                          </div>
                        );
                      })}
                    </div>
            ...
            <div style={{ position: "relative", marginBottom: 14 }}>
              <Search size={16} style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: PALETTE.muted }} />
              <input type="search" name="community-search" autoComplete="off" value={communityQuery} onChange={(e) => setCommunityQuery(e.target.value)} placeholder="Chercher un pseudo…" style={{ width: "100%", padding: "12px 16px 12px 38px", borderRadius: 14, border: "none", background: PALETTE.surface, color: PALETTE.text, fontSize: 14 }} />
            </div>
            ...
            >
              ...
              <span style={{ flex: 1, fontSize: 14, fontWeight: 700, color: PALETTE.text }}>{shareCopied ? "Lien copié !" : "Partager mon profil"}</span>
              {!shareCopied && <ChevronRight size={16} color={PALETTE.muted} />}
            </button>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: PALETTE.surface, borderTop: `1px solid ${PALETTE.line}`, display: "flex", alignItems: "center", justifyContent: "space-around", padding: "10px 12px calc(10px + env(safe-area-inset-bottom))", zIndex: 40 }}>
        {NAV_ITEMS.map((item) => {
          ...
          if (item.key === "add") {
            return (
              <button key={item.key} onClick={() => requireAuth(() => setAddMenuOpen(true))} className="st-btn" style={{ background: PALETTE.accent, border: "none", borderRadius: "50%", width: 52, height: 52, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", marginTop: -22, boxShadow: "0 6px 16px rgba(0,0,0,0.35)" }}>
        ...
        <div onClick={() => setAuthModalOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, zIndex: 70 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.surface, borderRadius: 22, padding: 26, maxWidth: 420, width: "100%", maxHeight: "85vh", overflowY: "auto", position: "relative" }}>
            ...
            <form onSubmit={handleAuthSubmit} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <input type="email" name="email" id="sortia-auth-email" autoComplete="email" required value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} placeholder="Adresse email" style={{ padding: "13px 16px", borderRadius: 14, border: `1px solid ${PALETTE.line}`, background: PALETTE.bg, color: PALETTE.text, fontSize: 14 }} />
              <input type="password" name="password" id="sortia-auth-password" autoComplete={authMode === "signup" ? "new-password" : "current-password"} required minLength={6} value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} placeholder="Mot de passe (6 caractères min.)" style={{ padding: "13px 16px", borderRadius: 14, border: `1px solid ${PALETTE.line}`, background: PALETTE.bg, color: PALETTE.text, fontSize: 14 }} />
      ...
      {/* Menu du bouton + : recherche ou découverte swipe */}
      {addMenuOpen && (
        <div onClick={() => setAddMenuOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 55 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.surface, borderRadius: "22px 22px 0 0", padding: 20, paddingBottom: "calc(20px + env(safe-area-inset-bottom))", maxWidth: 420, width: "100%", display: "flex", flexDirection: "column", gap: 10 }}>
            <button
              onClick={() => { setAddMenuOpen(false); setShowAddSearch(true); }}
              className="st-btn"
              style={{ display: "flex", alignItems: "center", gap: 14, background: PALETTE.surfaceAlt, border: "none", borderRadius: 16, padding: "16px 16px", cursor: "pointer", textAlign: "left" }}
            >
              <Search size={20} color={PALETTE.text} />
              <div>
                <div className="st-title" style={{ fontSize: 14, fontWeight: 700, color: PALETTE.text }}>Rechercher un anime</div>
                <div style={{ fontSize: 12, color: PALETTE.muted, marginTop: 1 }}>Tu sais déjà ce que tu cherches</div>
              </div>
            </button>
            <button
              onClick={() => { setAddMenuOpen(false); openDiscover(); }}
              className="st-btn"
              style={{ display: "flex", alignItems: "center", gap: 14, background: PALETTE.surfaceAlt, border: "none", borderRadius: 16, padding: "16px 16px", cursor: "pointer", textAlign: "left" }}
            >
              <Shuffle size={20} color={PALETTE.text} />
              <div>
                <div className="st-title" style={{ fontSize: 14, fontWeight: 700, color: PALETTE.text }}>Découvrir (swipe)</div>
                <div style={{ fontSize: 12, color: PALETTE.muted, marginTop: 1 }}>10 anime au hasard, à la Tinder</div>
              </div>
            </button>
          </div>
        </div>
      ...
      {/* Découverte façon swipe */}
      {discoverOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 65, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          {fontImport}
          <div style={{ width: "100%", maxWidth: 420, height: "min(760px, 92vh)", background: PALETTE.bg, borderRadius: 24, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }}>
          <div style={{ padding: "18px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <button onClick={() => setDiscoverOpen(false)} className="st-btn" style={{ background: PALETTE.surface, border: "none", borderRadius: 999, width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", color: PALETTE.text, cursor: "pointer" }}>
              <X size={18} />
            </button>
            <div className="st-title" style={{ fontSize: 15, fontWeight: 800 }}>
              {discoverDeck.length > 0 ? `${Math.min(discoverIndex + 1, discoverDeck.length)} / ${discoverDeck.length}` : "Découverte"}
            </div>
            <div style={{ width: 38 }} />
          </div>

          <div style={{ flex: 1, position: "relative", padding: "0 20px 20px", display: "flex", flexDirection: "column" }}>
            {discoverLoading && (
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: PALETTE.muted, fontSize: 14 }}>On te prépare des propositions…</div>
            )}

            {!discoverLoading && discoverError && (
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
                <div style={{ color: PALETTE.danger, fontSize: 14, fontWeight: 700, textAlign: "center" }}>{discoverError}</div>
                <button onClick={loadDiscoverDeck} className="st-btn" style={{ padding: "11px 22px", borderRadius: 999, border: "none", background: PALETTE.accent, color: PALETTE.accentText, fontWeight: 800, cursor: "pointer" }}>Réessayer</button>
              </div>
            )}

            {!discoverLoading && !discoverError && discoverIndex >= discoverDeck.length && discoverDeck.length > 0 && (
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 18, textAlign: "center" }}>
                <div style={{ fontSize: 40 }}>🎉</div>
                <div className="st-title" style={{ fontSize: 18, fontWeight: 800 }}>C'est tout pour cette fois !</div>
                <div style={{ fontSize: 13, color: PALETTE.muted, maxWidth: 260 }}>Tu as vu les 10 propositions. Relance pour une nouvelle sélection aléatoire.</div>
                <button onClick={loadDiscoverDeck} className="st-btn" style={{ padding: "13px 26px", borderRadius: 999, border: "none", background: PALETTE.accent, color: PALETTE.accentText, fontWeight: 800, fontFamily: "'Outfit', sans-serif", fontSize: 15, cursor: "pointer" }}>
                  Voir 10 autres anime
                </button>
              </div>
            )}

            {!discoverLoading && !discoverError && discoverDeck.length > 0 && discoverIndex < discoverDeck.length && (
              <>
                <div style={{ flex: 1, position: "relative" }}>
                  {[discoverDeck[discoverIndex + 1], discoverDeck[discoverIndex]].map((anime, i) =>
                    anime ? (
                      <div
                        key={anime.id}
                        onPointerDown={i === 1 ? onCardPointerDown : undefined}
                        onPointerMove={i === 1 ? onCardPointerMove : undefined}
                        onPointerUp={i === 1 ? onCardPointerUp : undefined}
                        onPointerLeave={i === 1 ? onCardPointerUp : undefined}
                        style={{
                          position: "absolute",
                          inset: 0,
                          borderRadius: 24,
                          ...
                          transform: i === 1 ? `translateX(${dragX}px) rotate(${dragX / 18}deg)` : "scale(0.95) translateY(10px)",
                          transition: i === 1 ? (dragAnim ? "transform 220ms ease" : "none") : "transform 160ms ease",
                          boxShadow: "0 10px 30px rgba(0,0,0,0.35)",
                          zIndex: i,
                        }}
                      >
                        <Poster image={anime.image} genre={anime.genre} />
                        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.85), rgba(0,0,0,0.05) 45%)" }} />
                        {i === 1 && (
                          <>
                            <div style={{ position: "absolute", top: 24, left: 20, padding: "6px 14px", borderRadius: 10, border: `3px solid ${PALETTE.ok}`, color: PALETTE.ok, fontWeight: 800, fontSize: 18, transform: "rotate(-16deg)", opacity: Math.min(1, Math.max(0, dragX / 80)) }}>
                              J'AIME
                            </div>
                            <div style={{ position: "absolute", top: 24, right: 20, padding: "6px 14px", borderRadius: 10, border: `3px solid ${PALETTE.danger}`, color: PALETTE.danger, fontWeight: 800, fontSize: 18, transform: "rotate(16deg)", opacity: Math.min(1, Math.max(0, -dragX / 80)) }}>
                              PASSE
                            </div>
                          </>
                        )}
                        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: 20 }}>
                          <div className="st-title" style={{ fontSize: 22, fontWeight: 800, color: "#fff" }}>{anime.title}</div>
                          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", marginTop: 4 }}>{anime.year} · {anime.genre}</div>
                        </div>
                      </div>
                    ) : null
                  )}
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 24, paddingTop: 18 }}>
                  <button onClick={() => flySwipe(-1)} className="st-btn" style={{ width: 56, height: 56, borderRadius: "50%", background: PALETTE.surface, border: `2px solid ${PALETTE.danger}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                    <X size={26} color={PALETTE.danger} />
                  </button>
                  <button onClick={() => flySwipe(1)} className="st-btn" style={{ width: 56, height: 56, borderRadius: "50%", background: PALETTE.surface, border: `2px solid ${PALETTE.ok}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                    <Heart size={24} color={PALETTE.ok} />
                  </button>
                </div>
              </>
            )}
      ...
      {/* Choix après un "j'aime" en mode découverte */}
      {discoverPending && (
        <div onClick={() => setDiscoverPending(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, zIndex: 70 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.surface, borderRadius: 22, padding: 24, maxWidth: 380, width: "100%", textAlign: "center" }}>
            <div style={{ width: 90, height: 126, borderRadius: 12, overflow: "hidden", margin: "0 auto 16px" }}>
              <Poster image={discoverPending.image} genre={discoverPending.genre} />
            </div>
            <div className="st-title" style={{ fontSize: 17, fontWeight: 800, marginBottom: 18 }}>{discoverPending.title}</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <button onClick={() => rateFromDiscover(discoverPending)} className="st-btn" style={{ padding: "13px 0", borderRadius: 999, border: "none", background: PALETTE.accent, color: PALETTE.accentText, fontWeight: 800, fontFamily: "'Outfit', sans-serif", fontSize: 15, cursor: "pointer" }}>
                Noter maintenant
              </button>
              <button onClick={() => quickAddToWatchlist(discoverPending)} className="st-btn" style={{ padding: "12px 0", borderRadius: 999, border: `1px solid ${PALETTE.line}`, background: "none", color: PALETTE.text, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                À voir plus tard
              </button>
            </div>
          </div>
        ...
        <div onClick={() => setShowAddSearch(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "60px 20px", zIndex: 50 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.surface, borderRadius: 20, padding: 18, maxWidth: 420, width: "100%", maxHeight: "70vh", overflowY: "auto" }}>
            <div style={{ position: "relative", marginBottom: 12 }}>
              <Search size={16} style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: PALETTE.muted }} />
              <input autoFocus type="search" name="anime-search-modal" autoComplete="off" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Chercher un anime à ajouter…" style={{ width: "100%", padding: "12px 16px 12px 38px", borderRadius: 14, border: "none", background: PALETTE.surfaceAlt, color: PALETTE.text, fontSize: 14 }} />
            </div>
