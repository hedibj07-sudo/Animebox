        ...
        {tab === "catalogue" && (
          <>
            <input
              ...
              style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1px solid ${PALETTE.line}`, background: PALETTE.surface, color: PALETTE.text, fontSize: 14, marginBottom: searchQuery.trim().length > 1 ? 10 : 20 }}
            />

            {searchQuery.trim().length > 1 && (
              <div style={{ border: `1px solid ${PALETTE.line}`, borderRadius: 12, background: PALETTE.surface, marginBottom: 24, overflow: "hidden" }}>
                {searchLoading && <div style={{ padding: 14, fontSize: 13, color: PALETTE.muted }}>Recherche…</div>}
                {!searchLoading && searchError && <div style={{ padding: 14, fontSize: 13, color: PALETTE.sakura }}>{searchError}</div>}
                {!searchLoading && !searchError && searchResults.map((r) => {
                  ...
                  return (
                    ...
                    >
                      {r.image ? (
                        <img src={r.image} alt="" style={{ width: 36, height: 50, objectFit: "cover", borderRadius: 4, flexShrink: 0 }} />
                      ) : (
                        <div style={{ width: 36, height: 50, borderRadius: 4, background: GENRE_GRADIENTS[r.genre] || GENRE_GRADIENTS.Anime, flexShrink: 0 }} />
                      )}
                      ...
                      {entry ? (
                        <span style={{ fontSize: 11, fontWeight: 800, color: PALETTE.ok, flexShrink: 0 }}>✓ noté</span>
                      ) : (
                        <span style={{ fontSize: 12, fontWeight: 700, color: PALETTE.sakura, flexShrink: 0 }}>Noter →</span>
                      )}
                    </button>
                  );
                })}
            ...
            {catalogLoading && topAnime.length === 0 && <div style={{ fontSize: 13, color: PALETTE.muted, marginBottom: 12 }}>Chargement du catalogue…</div>}
